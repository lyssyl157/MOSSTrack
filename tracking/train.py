...
def parse_args():
    ...
    parser.add_argument('--port', type=int, default='23456', help='Port of the current rank 0.')
    ...
    if args.mode == "single":
        ...
        train_cmd = "torchrun --nproc_per_node %d --master_port %d lib/train/run_training.py " \
                    ...
                    "--distill %d --script_teacher %s --config_teacher %s" % (
                        args.nproc_per_node, random.randint(10000, 50000),
...
#python tracking/train.py --script odtrack --config baseline --save_dir ./output --mode multiple --nproc_per_node 2 --use_wandb 0
