...
from lib.models.mosstrack.base_backbone import BaseBackbone
from transformers import BertTokenizer
from transformers.models.bert.modeling_bert import BertConfig, BertEmbeddings
from lib.models.mosstrack import utils as utils
from lib.models.mosstrack.utils import combine_tokens, recover_tokens
from lib.models.mosstrack.fusion_layer import FusionLayer
...
class Fast_iTPN(BaseBackbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.temporal_token_RGB_patch_pos_embed = PositionEmbeddingLearned(self.embed_dim, self.embed_dim)
        self.temporal_token_TIR_patch_pos_embed = PositionEmbeddingLearned(self.embed_dim, self.embed_dim)
        ...
        self.fusion_loc = [15,18,21]
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, len(self.fusion_loc))]  # stochastic depth decay rule
        self.fusion_layers = nn.ModuleList()
        if self.fusion_loc is not None and type(self.fusion_loc) == list:
            for i in range(len(self.fusion_loc)):
                self.fusion_layers.append(
                    FusionLayer(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                              drop=drop_rate,
    ...
    def create_mask(self, image, image_anno):
        ...
        x0 = (image_anno[:, 0] * width).unsqueeze(1).to(device)
        y0 = (image_anno[:, 1] * height).unsqueeze(1).to(device)
        w = (image_anno[:, 2] * width).unsqueeze(1).to(device)
        h = (image_anno[:, 3] * height).unsqueeze(1).to(device)
    ...
    def filter_tokens_by_connected_components(self, indices, H, W):
        filtered_batch = []
        for batch in range(indices.size(0)):
            mask = np.zeros((H, W), dtype=np.uint8)
            idx_list = indices[batch].cpu().numpy()
            ...
            if num <= 1:
                filtered = idx_list  # 只有一块，不需要过滤
            else:
                sizes = np.bincount(labeled.ravel())
                largest = sizes[1:].argmax() + 1  # 最大连通块ID
                filtered = [r * W + c for r in range(H) for c in range(W) if labeled[r, c] == largest]

            filtered_batch.append(torch.tensor(filtered, device=indices.device))

        # pad 到相同长度（防止 batch 内不等长）
        max_len = max(len(f) for f in filtered_batch)
        filtered_batch = [
            torch.cat([f, f.new_full((max_len - len(f),), f[len(f) // 2])])  # 用最后一个索引补齐
            for f in filtered_batch
        ]
        return torch.stack(filtered_batch, dim=0)

    def generate_temporal_tokens_init(self, z, template_anno_list, z_feat_RGB, z_feat_TIR):
        B = z.shape[0]
        z = z[:, -1:, :, :, :]
        z = z.view(-1, *z.size()[2:])

        # 获取标注并生成掩码
        z_anno = torch.stack(template_anno_list, dim=1)[:, -1:, :]
        z_anno = z_anno.view(-1, *z_anno.size()[2:])
        z_indicate_mask = self.create_mask(z, z_anno)

        # 转换为 Patch 级别的掩码 (类似 SUTrack 的多模态 Patch Embedding 处理 [cite: 131, 137])
        z_indicate_mask = z_indicate_mask.unfold(1, self.patch_size, self.patch_size).unfold(2, self.patch_size,
                                                                                             self.patch_size)
        z_indicate_mask = z_indicate_mask.mean(dim=(3, 4)).flatten(1)  # (B, 64) 假设 8x8 patches

        # 1. 计算每个样本中有效 patch 的个数
        counts_per_sample = (z_indicate_mask > 0).sum(dim=1)

        # 2. 找到当前 Batch 中能提取的最大安全 Token 数 (所有样本中的最小值)
        count = counts_per_sample.min().item()

            # 3. 核心修正：使用 topk 代替 nonzero，确保每个 Batch 提取的数量一致
        # 这样得到的 indices 形状直接就是 (B, count)
        _, indices = torch.topk(z_indicate_mask, k=count, dim=1)

        # 4. 对索引进行排序以保持空间顺序（可选，但通常对跟踪更稳健）
        indices = torch.sort(indices, dim=1)[0]

        # 5. 扩展维度以匹配特征维度 (B, count, 1)
        index_expanded = indices.unsqueeze(-1)

        # 6. 使用 gather 提取对应的特征 [cite: 145]
        # 注意：512 应根据你定义的 Embedding 维度 D 动态获取
        feat_dim = z_feat_RGB.shape[-1]
        temporal_token_RGB = torch.gather(z_feat_RGB, 1, index_expanded.expand(-1, -1, feat_dim))
        temporal_token_TIR = torch.gather(z_feat_TIR, 1, index_expanded.expand(-1, -1, feat_dim))

        return temporal_token_RGB, temporal_token_TIR, count

    def generate_temporal_tokens(self, attn_RGB, attn_TIR, x_feat_RGB, x_feat_TIR, temporal_token_num):
        lens_x = self.pos_embed_x.shape[1]

        attn_RGB = torch.mean(attn_RGB, dim=1)
        attn_TIR = torch.mean(attn_TIR, dim=1)
        RGB_l2s = attn_RGB[:, 0, -lens_x:]
        TIR_c2s = attn_TIR[:, 0, -lens_x:]

        _, index_RGB = torch.sort(RGB_l2s, dim=1, descending=True)
        _, index_TIR = torch.sort(TIR_c2s, dim=1, descending=True)
        top_index_RGB, _ = torch.sort(index_RGB[:, :temporal_token_num], dim=1)
        top_index_TIR, _ = torch.sort(index_TIR[:, :temporal_token_num], dim=1)
        union_index = torch.cat((top_index_RGB, top_index_TIR), dim=1)

        index_expanded = union_index.unsqueeze(2)
        temporal_token_RGB = torch.gather(x_feat_RGB, 1, index_expanded.expand(-1, -1, 512))
        temporal_token_TIR = torch.gather(x_feat_TIR, 1, index_expanded.expand(-1, -1, 512))

        return temporal_token_RGB, temporal_token_TIR

    def _z_feat(self,z, num_template):
    ...
    def _l_feat(self,l):
        if len(l) == 1:
            ...
            res = []
            for i in l:
                descript_id = self.tokenizer([i], add_special_tokens=True, truncation=True, padding="max_length", max_length=16)['input_ids']
                descript_id_tensor = torch.tensor(descript_id)
                i = self.descript_embedding(descript_id_tensor.to('cuda'))
                i += self.description_patch_pos_embed(i)
                res.append(i)
            return torch.stack(res, dim=1).squeeze()

    def _fusion_feat(self, z_RGB, x_RGB, z_TIR, x_TIR, B, temporal_token_len, l=None,temporal_query_RGB=None, temporal_query_TIR=None):
        # temporal_init = self.temporal_token.expand(B, 1, -1)
        # temporal_init = temporal_init + self.temporal_pos_embed
        temporal_init_RGB = self.temporal_token.expand(B, 1, -1)
        temporal_init_RGB = temporal_init_RGB + self.temporal_pos_embed

        temporal_init_TIR = self.temporal_token.expand(B, 1, -1)
        temporal_init_TIR = temporal_init_TIR + self.temporal_pos_embed

        x_RGB = combine_tokens(z_RGB, x_RGB, mode=self.cat_mode)
        x_TIR = combine_tokens(z_TIR, x_TIR, mode=self.cat_mode)

        temporal_query_RGB += self.temporal_token_RGB_patch_pos_embed(temporal_query_RGB)
        x_RGB = torch.cat([temporal_init_RGB, temporal_query_RGB, x_RGB], dim=1)

        temporal_query_TIR += self.temporal_token_TIR_patch_pos_embed(temporal_query_TIR)
        x_TIR = torch.cat([temporal_init_TIR, temporal_query_TIR, x_TIR], dim=1)
        ...
        fusion_index, i = 0, 0
        lens_z = z_RGB.shape[1]
        for blk in self.blocks[-self.num_main_blocks:]:
            x_RGB ,attn_RGB = blk(x_RGB)
            x_TIR, attn_TIR = blk(x_TIR)
            if self.fusion_loc is not None and i in self.fusion_loc:
                x_r_fusion, x_t_fusion, z_r_fusion, z_t_fusion = self.fusion_layers[fusion_index](
                                            x_RGB[:, 1 + temporal_token_len:, :],
                                            x_TIR[:, 1 + temporal_token_len:, :],
                                            temporal_query_RGB,
                                            temporal_query_TIR,
                                            lens_z)
                fusion_r = torch.cat([z_r_fusion,x_r_fusion], dim=1)
                fusion_t = torch.cat([z_t_fusion,x_t_fusion], dim=1)

                x_RGB = x_RGB.clone()
                x_TIR = x_TIR.clone()

                x_RGB_part = x_RGB[:, 1 + temporal_token_len:, :] + fusion_r
                x_TIR_part = x_TIR[:, 1 + temporal_token_len:, :] + fusion_t
                x_RGB = torch.cat([
                    # x_RGB[:, :1 + l.shape[1] + temporal_token_len, :],
                    x_RGB[:, :1 + temporal_token_len, :],
                    x_RGB_part
                ], dim=1)
                x_TIR = torch.cat([
        ...
        return x_RGB, attn_RGB, x_TIR, attn_TIR

    def forward_features(self, template_list, search, template_anno_list, pred_boxes, temporal_query_RGB=None, temporal_query_TIR=None):
        B = search.size(0)
        num_template = len(template_list)
        z = torch.stack(template_list, dim=1)  # (b,n,c,h,w)
        x = search.unsqueeze(1)                # (b,n,c,h,w)
        z_RGB = z[:, :, :3, :, :]  # (b,n,c,h,w)
        x_RGB = x[:, :, :3, :, :]  # (b,n,c,h,w)
        z_TIR = z[:, :, 3:, :, :]  # (b,n,c,h,w)
        x_TIR = x[:, :, 3:, :, :]  # (b,n,c,h,w)
        x_RGB = x_RGB.view(-1, *x_RGB.size()[2:])  # (bn,c,h,w)
        z_RGB = z_RGB.view(-1, *z_RGB.size()[2:])  # (bn,c,h,w)
        x_TIR = x_TIR.view(-1, *x_TIR.size()[2:])  # (bn,c,h,w)
        z_TIR = z_TIR.view(-1, *z_TIR.size()[2:])  # (bn,c,h,w)

        # add indicate_embeddings to z and x
        z_feat_RGB = self._z_feat(z_RGB, num_template)
        z_feat_TIR = self._z_feat(z_TIR, num_template)
        x_feat_RGB = self._x_feat(x_RGB)
        x_feat_TIR = self._x_feat(x_TIR)
        # l_feat = self._l_feat(l)

        if temporal_query_RGB is None:
            temporal_query_RGB, temporal_query_TIR, temporal_token_num\
                = self.generate_temporal_tokens_init(z[:, :, :3, :, :], template_anno_list, z_feat_RGB[:, self.pos_embed_z.shape[1]:, :], z_feat_TIR[:, self.pos_embed_z.shape[1]:, :])
            temporal_token_len = temporal_token_num
        else:
            temporal_token_num = math.ceil(sum(self.pos_embed_x.shape[1] * pred_boxes[:, 2] * pred_boxes[:, 3])/B)
            temporal_token_len = temporal_query_RGB.shape[1]

        l_feat=None
        fusion_feat_RGB, attn_RGB, fusion_feat_TIR, attn_TIR\
            = self._fusion_feat(z_feat_RGB, x_feat_RGB, z_feat_TIR, x_feat_TIR, B, temporal_token_len, l_feat, temporal_query_RGB, temporal_query_TIR)
        temporal_token_RGB, temporal_token_TIR\
            = self.generate_temporal_tokens(attn_RGB, attn_TIR, x_feat_RGB, x_feat_TIR, temporal_token_num)


        aux_dict = {"attn_RGB": attn_RGB,
                    "attn_TIR": attn_TIR,
                    "temproal_token_RGB": temporal_token_RGB,
                    "temporal_token_TIR": temporal_token_TIR}

        return fusion_feat_RGB, fusion_feat_TIR, aux_dict

    def forward(self, template_list, search_list, template_anno_list, pred_boxes, temporal_query_RGB, temporal_query_TIR, **kwargs):
        """
        Joint feature extraction and relation modeling for the basic ViT backbone.
        Args:
            z (torch.Tensor): template feature, [B, C, H_z, W_z]
            x (torch.Tensor): search region feature, [B, C, H_x, W_x]
            l (list.str): descript of search region

        Returns:
            x (torch.Tensor): merged template and search region feature, [B, L_z+L_x, C]
            attn : None
        """
        # x_rgb, x_tir, aux_dict = self.forward_features(z_RGB, z_TIR, x_RGB, x_TIR, l, temporal_query, top_K)
        x, x_t, aux_dict = self.forward_features(template_list, search_list, template_anno_list, pred_boxes, temporal_query_RGB, temporal_query_TIR)

        return x, x_t, aux_dict
